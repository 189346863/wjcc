...
// 目标：继续使用 Workers KV 存储文件；文件列表公开；公开文件可下载/引用；私密文件可见但不可下载/引用；上传、删除、移动、切换公开私密、生成外链、修改配置需要后端密码校验。
...
async function ensureAdminPassword(text){var pwd=await askPassword(text);if(!pwd)return null;var data=await verifyPassword(pwd);if(data.success)return pwd;showToast('密码错误');return null;}
...
async function fetchFiles(){try{var res=await fetch('/api/files');var data=await res.json().catch(function(){return {success:false,error:'文件列表接口返回异常'};});if(!res.ok||data.error){throw new Error(data.error||('文件列表请求失败：'+res.status));}return data.files||[];}catch(e){showToast('文件列表错误：'+(e.message||e));return [];} }async function getStats(){try{var res=await fetch('/api/stats');var data=await res.json().catch(function(){return {totalFiles:0,totalSize:'0 B',totalDownloads:0,error:'统计接口返回异常'};});if(!res.ok||data.error){throw new Error(data.error||('统计请求失败：'+res.status));}return data;}catch(e){showToast('统计接口错误：'+(e.message||e));return {totalFiles:0,totalSize:'0 B',totalDownloads:0};}}
...
function renderFileList(){var filtered=allFiles.slice();var search=$('searchInput').value.trim().toLowerCase();var folder=$('folderSelect').value;if(search)filtered=filtered.filter(function(f){return String(f.name||'').toLowerCase().indexOf(search)>=0;});if(folder)filtered=filtered.filter(function(f){return f.folder===folder;});var container=$('fileList');container.textContent='';if(!filtered.length){var empty=document.createElement('div');empty.className='empty';empty.textContent='📭 暂无文件';container.appendChild(empty);return;}filtered.forEach(function(f){var isPrivate=f.visibility==='private';var previewUrl='/api/download/'+encodeURIComponent(f.id);var div=document.createElement('div');div.className='file-item'+(isPrivate?' private':'');var cb=document.createElement('input');cb.type='checkbox';cb.className='file-checkbox';cb.checked=selectedFiles.has(f.id);cb.onchange=function(){if(cb.checked)selectedFiles.add(f.id);else selectedFiles.delete(f.id);};div.appendChild(cb);var preview=document.createElement('div');preview.className='file-preview';if(isPrivate){preview.onclick=function(){showToast('私密文件不可引用或下载，请先设为公开');};var lock=document.createElement('div');lock.className='file-preview-placeholder';lock.textContent='🔒';preview.appendChild(lock);}else{preview.onclick=function(){window.open(previewUrl,'_blank');};if(isImageFile(f.name)){var img=document.createElement('img');img.className='file-preview-img';img.src=previewUrl;img.alt='预览';preview.appendChild(img);}else{var ph=document.createElement('div');ph.className='file-preview-placeholder';ph.textContent=getFileIcon(f.name);preview.appendChild(ph);}}div.appendChild(preview);var name=document.createElement('div');name.className='file-name';name.textContent=f.name||f.id;div.appendChild(name);var badge=document.createElement('div');badge.className='privacy-badge '+(isPrivate?'private':'public');badge.textContent=isPrivate?'🔒 私密：可见，但不可引用/下载':'🌐 公开：可引用/下载';var badgeWrap=document.createElement('div');badgeWrap.style.textAlign='center';badgeWrap.appendChild(badge);div.appendChild(badgeWrap);var meta=document.createElement('div');meta.className='file-meta';meta.textContent=(f.size||'0 B')+' · '+(f.time||'')+' · 下载'+(f.downloads||0)+'次';div.appendChild(meta);var actions=document.createElement('div');actions.className='file-actions';actions.appendChild(makeButton('⬇️ 下载','btn-download',function(){if(isPrivate){showToast('私密文件不可下载，请先设为公开');return;}recordDownload(f.id);window.open(previewUrl,'_blank');}));actions.appendChild(makeButton('🔗 外链','btn-share',function(){if(isPrivate){showToast('私密文件不可生成外链，请先设为公开');return;}currentShareFile=f.id;$('shareModal').style.display='flex';$('shareLinkBox').style.display='none';}));actions.appendChild(makeButton(isPrivate?'🌐 设公开':'🔒 设私密','btn-privacy',function(){toggleVisibility(f.id,f.visibility||'public');}));actions.appendChild(makeButton('📁 移动','btn-folder',function(){openFolderModal(f.id);}));actions.appendChild(makeButton('🗑️ 删除','btn-delete',function(){deleteFile(f.id);}));div.appendChild(actions);container.appendChild(div);});}
async function recordDownload(fileId){try{await fetch('/api/download/'+encodeURIComponent(fileId)+'/record',{method:'POST'});}catch(e){}}
async function refreshAll(){try{allFiles=await fetchFiles();loadFolders();renderFileList();await loadStats();}catch(e){var c=$('fileList');if(c){c.textContent='';var d=document.createElement('div');d.className='empty';d.textContent='页面已打开，但接口加载失败：'+(e&&e.message?e.message:String(e));c.appendChild(d);}showToast('接口加载失败，请打开 /__health 检查');}}
...
async function toggleVisibility(fileId,currentVisibility){var next=currentVisibility==='private'?'public':'private';var text=next==='private'?'设为私密后，该文件仍在列表可见，但不能被引用、外链访问或下载。确认继续？':'设为公开后，该文件可以被引用、外链访问和下载。确认继续？';if(!confirm(text))return;var pwd=await ensureAdminPassword('切换公开/私密需要管理密码');if(!pwd)return;var data=await apiJson('/api/visibility',{id:fileId,visibility:next,password:pwd});if(data.success){showToast(next==='private'?'已设为私密':'已设为公开');await refreshAll();}else{if(data.error&&data.error.indexOf('密码')>=0)clearAdminPassword();showToast(data.error||'切换失败');}}
function batchDownload(){if(selectedFiles.size===0){showToast('请先选择文件');return;}var ids=Array.from(selectedFiles).filter(function(id){var f=allFiles.find(function(x){return x.id===id;});return !f||f.visibility!=='private';});if(ids.length===0){showToast('选中的都是私密文件，不能下载');return;}if(ids.length<selectedFiles.size)showToast('已跳过私密文件');window.open('/api/batch-download?ids='+encodeURIComponent(JSON.stringify(ids)),'_blank');}
...
function escapeMarkdownName(name){return String(name||'file').replace(/./g,function(ch){return (ch==='['||ch===']'||ch==='('||ch===')'||ch.charCodeAt(0)===92)?String.fromCharCode(92)+ch:ch;});}
...
async function init(){try{await loadLogo();bindEvents();await refreshAll();}catch(e){showToast('初始化失败：'+(e&&e.message?e.message:String(e)));}}init();
</script>
</body>
</html>`;
...
export default {
  async fetch(request, env, ctx) {
    try {
      ...
      const kv = new Proxy({}, {
        get(_target, prop) {
          const real = __kv || (__kv = getKV(env));
      ...
      if (path === "/__health") {
        ...
        let kvError = "";
        try {
          const realKv = getKV(env);
          await realKv.list({ limit: 1 });
        } catch (e) {
          kvStatus = "ERROR";
          kvError = e && e.message ? e.message : String(e);
        }
        const envKeys = Object.keys(env || {}).sort().join(", ") || "无";
        return htmlResponse(`<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>健康检查</title><style>body{font-family:system-ui;background:#0f172a;color:#e2e8f0;padding:24px;line-height:1.7}.card{max-width:760px;margin:auto;background:#111827;border:1px solid #334155;border-radius:18px;padding:22px}code{background:#020617;padding:2px 6px;border-radius:6px}.ok{color:#86efac}.err{color:#fca5a5}</style></head><body><div class="card"><h1>Pages KV 云盘健康检查</h1><p>Worker：<span class="ok">已运行</span></p><p>KV 绑定 <code>music_kv</code>：<span class="${kvStatus === "OK" ? "ok" : "err"}">${kvStatus}</span></p><p>错误信息：<code>${escapeHtml(kvError || "无")}</code></p><p>当前 env keys：<code>${escapeHtml(envKeys)}</code></p><p>如果 KV 是 ERROR，请在 Pages 项目的 Settings → Bindings 添加 KV namespace，变量名必须是 <code>music_kv</code>，然后重新部署。</p><p><a style="color:#93c5fd" href="/">返回首页</a></p></div></body></html>`);
      }
      ...
      if (recordMatch && request.method === "POST") {
        ...
        if (metadata && metadata.visibility === "private") return jsonResponse({ success: false, error: "私密文件不可下载" }, { status: 403 });
      ...
      if (path === "/api/batch-download") {
        ...
        for (const id of ids) {
          ...
          if (metadata && metadata.visibility === "private") {
            rows.push(`<p>🔒 私密文件不可下载：${escapeHtml(metadata.name || id)}</p>`);
            continue;
          }
          const name = metadata && metadata.name ? metadata.name : id;
          const href = "/api/download/" + encodeURIComponent(id);
          rows.push(`<p><a href="${href}" target="_blank" rel="noopener">下载：${escapeHtml(name)}</a></p>`);
        }
        const links = rows.join("\n");
      ...
      if (path.startsWith("/api/download/")) {
        ...
        const metadata = await getFileMetadata(kv, fileId);
        if (metadata && metadata.visibility === "private") return textResponse("该文件为私密文件，不能被引用或下载", { status: 403 });
      ...
      if (path === "/api/share" && request.method === "POST") {
        ...
        const metadata = await getFileMetadata(kv, id);
        if (!metadata) return jsonResponse({ success: false, error: "文件不存在" }, { status: 404 });
        if (metadata.visibility === "private") return jsonResponse({ success: false, error: "私密文件不能生成外链，请先设为公开" }, { status: 403 });
      ...
      if (path.startsWith("/s/")) {
        ...
        if (metadata && metadata.visibility === "private") return textResponse("该文件已被设为私密，分享链接不可访问", { status: 403 });
      ...
      if (path === "/api/visibility" && request.method === "POST") {
        const body = await readJson(request);
        if (!body) return jsonResponse({ success: false, error: "请求体不是有效 JSON" }, { status: 400 });
        const ok = await verifyAdminPassword(env, body.password || getPasswordFromHeader(request));
        if (!ok) return jsonResponse({ success: false, error: "密码错误" }, { status: 403 });
        ...
        if (!visibility) return jsonResponse({ success: false, error: "权限类型只能是 public 或 private" }, { status: 400 });
        const metadata = await getFileMetadata(kv, id);
        if (!metadata) return jsonResponse({ success: false, error: "文件不存在" }, { status: 404 });
        ...
        if (visibility === "private") {
          deletedShares = await deleteSharesForFile(kv, id);
        }
        return jsonResponse({ success: true, visibility, deletedShares });
      }
      ...
      const message = "服务器错误：" + (err && err.message ? err.message : String(err));
      const wantsHtml = (request.headers.get("Accept") || "").includes("text/html");
      if (wantsHtml) {
        return htmlResponse(`<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Worker 错误</title><style>body{font-family:system-ui;background:#0f172a;color:#e2e8f0;padding:24px;line-height:1.7}.card{max-width:760px;margin:auto;background:#111827;border:1px solid #334155;border-radius:18px;padding:22px}code{background:#020617;padding:2px 6px;border-radius:6px}.err{color:#fca5a5}a{color:#93c5fd}</style></head><body><div class="card"><h1 class="err">页面后端运行错误</h1><p><code>${escapeHtml(message)}</code></p><p>请先打开 <a href="/__health">/__health</a> 检查 KV 绑定是否为 <code>music_kv</code>。</p><p>如果刚修改过绑定或环境变量，请重新上传 zip 部署一次。</p></div></body></html>`, { status: 500 });
      }
